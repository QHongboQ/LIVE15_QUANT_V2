...
def test_capture_boundary_is_a_storage_sibling_without_hot_store_coupling() -> None:
    boundary = (
        ROOT / "src" / "live15_quant_v2" / "data" / "storage" / "capture_boundary"
    )
    source = "\n".join(
...
def test_questdb_imports_are_contained_inside_provider_specific_adapters() -> None:
    imports = []
    for source in (ROOT / "src" / "live15_quant_v2").rglob("*.py"):
        if re.search(
            r"^import questdb$", source.read_text(encoding="utf-8"), re.MULTILINE
        ):
    ...
    assert imports == [
        ...
        "src/live15_quant_v2/data/replay_as_of/questdb_source.py",
