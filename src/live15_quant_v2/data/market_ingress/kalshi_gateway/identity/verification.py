"""Fail-closed verification of one exact official identity."""
...
class OfficialMarketVerifier:
    def verify(self,*,scope:MarketScopePort,binding:MarketScopeBinding,window:MarketWindow,markets:Iterable[DiscoveredMarket])->VerificationResult:
        ...
        exact=[m for m in markets if m.binding==binding and m.open_time==window.open_time and m.close_time==window.close_time]
        ...
        market=exact[0]
        if not market.event_ticker or not market.ticker.startswith(market.event_ticker): return VerificationResult(VerificationStatus.INVALID,reason="incoherent ticker/event")
        ...
        return VerificationResult(VerificationStatus.VERIFIED,VerifiedMarketIdentity(binding,market.ticker,market.event_ticker,window,market.strike))