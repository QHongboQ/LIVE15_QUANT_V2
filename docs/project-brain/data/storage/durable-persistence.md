...
`CaptureFact` identity is physically idempotent. The sealed Hot Store evolution
implements the approved physical configuration for a future new table; it does
not materialize or activate the canonical table.

Any future canonical-table activation remains separately authorized work. Before
that activation, perform read-only audits for duplicate
...
│   └── native physical transport-idempotency / DEDUP sealed
└── Durable Persistence
